...
import type { ConfigType, RateLimitKeys, RateStateLimitType } from "../Types/HelperTypes.js";
import {
	POE_API_BASE_URL,
	...
	private parseRateLimitHeaders(res: AxiosResponse): RateStateLimitType {
		...
		for (const [header, mappedHeader] of Object.entries(headerMappings)) {
			if (headers[header.toLocaleLowerCase()]) {
				state[mappedHeader] = headers[header].split(",").map((el: string) => el.split(":").map(Number));
			}
		}
		return state;
	}
